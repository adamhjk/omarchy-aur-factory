...
- **Existing PKGBUILD** (checksums → installdeps → isolated build → lint → audit, auto-retried on audit failure): `swamp workflow run build-package --input dir=<abs pkgdir> --input name=<pkgname> --input version=<pkgver>-<pkgrel>`
...
- **Vet only**: `swamp workflow run vet-package` (same inputs as build-package). This is a separate, unrelated-to-retries entry point for a one-off lint+audit — `build-package` no longer calls it internally (see note below on why).
- **Audit failures self-heal**: if the built package fails audit, `build-package` automatically calls the packager `fix` method — a constrained Claude call that patches the PKGBUILD in place from the audit (and lint) findings — then redoes checksums → installdeps → build → lint → audit. Capped at 3 total build attempts (1 initial + 2 auto-fix retries); if it's still failing after attempt 3, the workflow fails and the audit/lint evidence + `fix-<name>-<version>-attempt<n>` evidence show what was tried. Lint failures alone are not retried (only audit failures trigger a fix), but the final lint state still gates overall success. `build-package`'s attempt loop is unrolled as static steps per attempt (`checksums-2`, `build-2`, `fix-2`, …) rather than a real loop — swamp workflow DAGs don't support cycles — and lint/audit run as direct steps per attempt rather than via a nested `vet-package` call, because nested `type: workflow` steps don't invalidate the parent's `data.latest()` cache (a real engine gap — a nested workflow's own writes are correctly persisted but invisible to the parent's guards/asserts for the rest of that run).
- Evidence keys are `<stage>-<pkgname>-<pkgver>-<pkgrel>`: `swamp data get packager lint-sl-5.02-1`, `buildlog-…`, `pkgbuild-…` (snapshot), `note-<pkgname>-<stage>` (design rationale — record yours with the `note` method after hand-editing a PKGBUILD), `fix-<pkgname>-<pkgver>-<pkgrel>-attempt<n>` / `fixlog-…-attempt<n>` (audit-retry Claude calls).
- Each run generates a dossier: `swamp report get @omarchy/package-dossier --workflow <workflow> --markdown` (run-scoped; only the latest survives). The durable per-package dossier is packager data: `swamp data get packager dossier-<pkgname>-<version>` — build-package's `finalize` job stores it on success AND failure, including an "Audit-retry fixes" section when a fix ran.
- Failed build after 3 auto-fix attempts? Read the stage evidence/logs (especially `fix-<name>-<version>-attempt*` and `auditlog-…`), then retry with maintainer hints: the `create-package` workflow takes an optional `hints` input that is injected into the authoring prompt (the app's retry endpoint records the hint on the request via `record-retry` first).
